...
import beast.Leo.BeastInterfaceSerializable;
...
* is a very very good reason. */
    ...
    public Runnable parseFile(final File file) throws SAXException, IOException, ParserConfigurationException, XMLParserException {
    	return parseFile(file, false, this.m_bmcmc);
    }
    
    public Runnable parseFile(final File file, boolean sampleFromPrior, BeastMCMC bmc) throws SAXException, IOException, ParserConfigurationException, XMLParserException {
        ...
        try {
        	for (int i = 0; i < beastObjectsWaitingToInit.size(); i++) {
        		...
        		{// initialization of random tree times
        			int yull;
        					yull=3;
        					RandomTree rtree=(RandomTree)beastObject;
        		...
        		{// initialization of gammashape: we sample and calculate the logp
        			int yull;
        			yull=3;
        			Prior pr=(Prior)beastObject;
        			Random random = new Random(Randomizer.getSeed());
        			pr.sample(null, random);
        			pr.calculateLogP();
        			yull=5;
        		...
        		{// for the effective population size
         		...
         		//	for(int inneri=0; inneri<dim; inneri++)
         			{// should be just one dimension but we keep it general in n dimensions
                        //double val=thisparam.getArrayValue();
             			//thisparam.set
             			double lambda=0.33;
             			Exponential expn = new Exponential();
             			expn.lambdaInput.setValue(lambda + "", expn);
             			expn.initAndValidate();
             			Double[][] smp=expn.sample(1);
             			double Neff;
             			Neff=smp[0][0];
             			thisparam.setValue(0, Neff); // first dimension, set to Neff
            			m_bmcmc.m_initPopSize=Neff;
         			}
         			//Double objval=(Double)beastObject.getInputValue("value");
         			//Object objvalues= beastObject.getInputValue("values");
         			//Object objval= beastObject.getInputValue("value");
         			//objval[0].value=0;
         			int j;
         			j=3;
         			//beastObject.setInputValue("values", Neff);
         					//beastObject.setInputValue(name, value);    ( name, value);
        		}
        		int j;
        		j=3;
